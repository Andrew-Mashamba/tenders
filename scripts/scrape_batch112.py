...
Run ID: run_20260313_205329_batch112
"""
import json
import re
import subprocess
...
RUN_ID = "run_20260313_205329_batch112"

INSTITUTIONS = [
    "tact", "tadb", "tadiye", "taec", "taffa",
    "tafiri", "tafori", "tag", "taha", "tahaceramics",
    "tahafresh", "tahea", "tahfif", "tai", "tai-adventures",
    "taifamining", "taima", "tajpi", "talenthub", "talia",
    "taliri", "tall", "tama", "tambuautu", "tamisemim",
]

DOC_EXTENSIONS = (".pdf", ".doc", ".docx", ".xls", ".xlsx", ".zip")
TENDER_KEYWORDS = re.compile(
    r"\b(tender|tenders|zabuni|rfp|rfq|eoi|rfi|manunuzi|invitation to bid|procurement)\b",
    re.I
)
...
def load_readme(slug):
    ...
    if text.startswith("---"):
        ...
        if len(parts) >= 3:
            fm = parts[1]
            tender_url = re.search(r'tender_url:\s*["\']?([^"\'\n]+)["\']?', fm)
            homepage = re.search(r'homepage:\s*["\']?([^"\'\n]+)["\']?', fm)
            inst_name = re.search(r'name:\s*["\']([^"\']+)["\']', fm)
            email = re.search(r'email:\s*["\']?([^\s"\'\n]+)["\']?', fm)
            url = (tender_url.group(1).strip() if tender_url else "") or (homepage.group(1).strip() if homepage else "")
            return (
                url if url else None,
                inst_name.group(1).strip() if inst_name else slug,
...
def fetch_url(url, timeout=30):
    try:
        result = subprocess.run(
            ["curl", "-s", "-L", "-k", "-A", "Mozilla/5.0 (compatible; TendersBot/1.0)", "--max-time", str(timeout), url],
            capture_output=True,
            text=True,
            timeout=timeout + 5,
        )
...
def extract_emails(html):
    ...
    exclude = ["example.com", "sentry.io", "wixpress", "schema.org", "yimg.com", "google.com", "facebook.com", "twitter.com", "linkedin.com", "youtube.com", "gravatar", "w3.org", "bloxic.com", "sample@mail.com"]
    return sorted([e for e in emails if not any(x in e.lower() for x in exclude)])
...
def scrape_institution(slug):
    ...
    if isinstance(contact, dict):
        if contact.get("email"):
            contact_emails.append(str(contact["email"]).strip())
        contact_emails.extend([str(e).strip() for e in contact.get("alternate_emails", []) if e])
    all_emails = list(dict.fromkeys([e for e in contact_emails + emails if e and "@" in e]))
    if has_tenders and doc_links:
        seq = 1
        tender_id = f"{slug.upper().replace('-','')}-2026-{seq:03d}"
        ...
        for url in doc_links[:20]:
            try:
                ...
                subprocess.run(
                    ["curl", "-s", "-L", "-k", "-o", str(out_path), "-A", "Mozilla/5.0", "--max-time", "60", url],
                    capture_output=True,
                    timeout=65,
                )
        ...
        display_name = inst_name or slug.replace("-", " ").title()
        lead = {
            ...
            "opportunity_description": f"No formal tenders found on {display_name} website. ZIMA Solutions could offer digital transformation, fintech integrations, or ICT services.",
            "draft_email_subject": f"Partnership Opportunity – ZIMA Solutions & {display_name}",
            "draft_email_body": f"Dear {display_name} Team,\n\nZIMA Solutions Limited specialises in digital transformation for financial institutions and enterprises in Tanzania. We noticed your organisation and believe we could support your technology and innovation goals.\n\nOur offerings include:\n• GePG, TIPS, and RTGS integrations\n• SACCO and microfinance systems\n• AI-powered customer engagement\n• HR, school, and healthcare management systems\n\nWe would welcome a conversation about how we might support {display_name}. Could we schedule a brief call?\n\nBest regards,\nZIMA Solutions Limited\ninfo@zima.co.tz | +255 69 241 0353\n",
