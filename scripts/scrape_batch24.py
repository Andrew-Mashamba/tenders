#!/usr/bin/env python3
"""
Scrape 26 institutions for active tenders.
Run ID: run_20260315_060430_batch24
"""
import json
import re
import subprocess
...
RUN_ID = "run_20260315_060430_batch24"
INSTITUTIONS = [
    "conveyance", "coop-bank", "cordex", "coreit", "coresecurities", "cornerstone",
    "cosota", "costbenchmark", "cotech", "cottonclub", "counsenuth", "coyesa",
    "coyeta", "cpat", "cpb", "cpctanzania", "cpi", "cps", "cqs", "craftmasterbuilders",
    "cranetrader", "cravason", "crb", "crbafricalegal", "crdb",
]
...
def process_institution(slug):
    ...
    # counsenuth: tender_url is a PDF - fetch homepage for links and include PDF as doc
    fetch_url_actual = tender_url
    if slug == "counsenuth" and tender_url.endswith(".pdf"):
        parsed = urlparse(tender_url)
        base = f"{parsed.scheme}://{parsed.netloc}"
        fetch_url_actual = base + "/"
    ...
    # counsenuth: add the PDF tender_url as a document if it's a direct PDF
    if slug == "counsenuth" and tender_url.endswith(".pdf") and not tenders:
        tenders = [{
            "tender_id": "COUNSENUTH-2026-001",
            "title": "COUNSENUTH Annual Procurement Advert 2026",
            "published_date": datetime.now(timezone.utc).strftime("%Y-%m-%d"),
            "closing_date": "",
            "document_links": [tender_url],
            "source_url": tender_url,
        }]
    elif slug == "counsenuth" and tender_url.endswith(".pdf") and tenders:
        # Add PDF to document_links if not already there
        for t in tenders:
            if tender_url not in t.get("document_links", []):
                t.setdefault("document_links", []).append(tender_url)
    ...
    if tenders:
        for t in tenders:
            ...
            tender_json = {
                ...
                "documents": [{"original_url": u, "local_path": str(inst_dir / "downloads" / tid / "original" / (u.split("/")[-1].split("?")[0].replace("%20", "_") or "doc.pdf"))} for u in docs],
