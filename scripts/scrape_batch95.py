#!/usr/bin/env python3
"""Scrape 25 institutions for active tenders. Run ID: run_20260315_060430_batch95"""
import json
import re
import subprocess
import sys
...
RUN_ID = "run_20260315_060430_batch95"

INSTITUTIONS = [
    ("qualtrust", "Qualtrust Company Limited", "http://www.qualtrust.co.tz/"),
    ("quantum", "QI Group | Marketing, Media & Tech", "https://qigroup.tz/"),
    ("quantumarchitects", "Quantum Space Architects", "https://quantumarchitects.co.tz/"),
    ("quantumresearch", "quantumresearch", "https://quantumresearch.co.tz/"),
    ("queensgem", "Queens Gem & Jewellery", "https://www.queensgem.co.tz/"),
    ("quickstepinvestment", "Quick Step Investment", "https://quickstepinvestment.co.tz/"),
    ("quixgroup", "Quix Group", "https://quixgroup.co.tz/"),
    ("raawu", "RAAWU", "https://raawu.or.tz/"),
    ("rabikafarm", "Rabika Farm", "https://rabikafarm.co.tz/"),
    ("radarsecurity", "Radar Security - Tanzania", "https://radarsecurity.co.tz/"),
    ("radian", "Radian | Integrated Marketing And Technology", "https://www.radian.co.tz/"),
    ("radio1", "RadioOne | Stereo", "https://radio1.co.tz/"),
    ("radio5fm", "RADIO 5 FM", "https://www.radio5fm.co.tz/ppaa-yaokoa-bilioni-586-5-yazima-zabuni-43-hewa/"),
    ("radiojoyfm", "JOY FM", "https://radiojoyfm.co.tz/dc-kasulu-akabidhiwa-mradi-wenye-thamani-ya-milioni-35-na-nrc/"),
    ("radiokicheko", "Radio Kicheko Live", "https://radiokicheko.co.tz/"),
    ("radiokwizera", "radiokwizera.co.tz", "https://radiokwizera.co.tz/"),
    ("radiowave", "Radiowave Communications LTD", "https://radiowave.co.tz/"),
    ("rafikiattorneys", "Rafiki Attorneys", "https://rafikiattorneys.co.tz/"),
    ("rafikisdo", "RAFIKI-SDO | Centre for Youth & Children's Rights", "https://rafikisdo.or.tz/tender.html"),
    ("rahamiagri", "Rahami Agri Group", "https://www.rahamiagri.co.tz/"),
    ...
    ("raitech", "Rai Technologies", "https://raitech.co.tz/"),
    ("raktizm", "RAKTIZIM GROUP LIMITED", "https://raktizm.co.tz/"),
]

TENDER_KEYWORDS = re.compile(
    r"\b(tender|tenders|procurement|rfp|rfi|eoi|expression of interest|"
    r"request for proposal|request for quotation|zabuni|manunuzi|bid\s+document)\b",
    re.I
)
...
def append_scrape_log(inst_dir: Path, run: dict) -> None:
    ...
    run_entry = {
        ...
        "timestamp": run.get("timestamp", datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")),
        "duration_seconds": run.get("duration_seconds", 0),
        "status": run.get("status", ""),
        "tenders_found": run.get("tenders_found", 0),
...
def process_institution(slug: str, name: str, url: str) -> tuple[str, int, int]:
    ...
    if has_tenders:
        ...
        if docs:
            ...
            now = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
            document_records = []
            for doc_url, filename in docs[:15]:
    ...
    # No tenders: add lead (append_lead skips if already present)
