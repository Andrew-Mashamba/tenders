...
Run ID: run_20260315_060430_batch108
"""
import json
import re
import subprocess
...
RUN_ID = "run_20260315_060430_batch108"
...
INSTITUTIONS = [
    ...
    ("somaconnect", "https://somaconnect.or.tz/"),
    ...
    ("sphousing", "https://sphousing.co.tz/"),
]
...
def get_url_from_readme(slug: str) -> str | None:
    """Get tender_url from README if available."""
...
def scrape_institution(slug: str, url: str) -> tuple[str, int, int]:
    ...
    if not inst_dir.exists():
        return "error", 0, 0  # dir not found - caller will use default url

    readme_url = get_url_from_readme(slug)
    if readme_url:
        url = readme_url
    ...
    if not tenders:
        ...
        emails = list(dict.fromkeys(e for e in emails if not any(x in e.lower() for x in (".png", ".jpg", "example.com", "wixpress", "sentry", "ex.com"))))[:10]
...
def main():
    for i, (slug, url) in enumerate(INSTITUTIONS):
        ...
        inst_dir = PROJECT / "institutions" / slug
        if not inst_dir.exists():
            print(f"RESULT|{slug}|error|0|0", flush=True)
            continue
        try:
